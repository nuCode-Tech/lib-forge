...
2. **Compute the `build_id`.** LibForge derives the release hash (`b1-<digest>`) from `Cargo.toml`, `Cargo.lock`, any `libforge.yaml`, and optional `.udl` files. This deterministic hash ties the manifest and artifacts to the exact source and configuration that produced them.
