...
use xforge_cli::commands::{build, bundle, publish};
...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let stamp = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    path.push(format!("xforge-e2e-github-{}-{}", name, stamp));
...
fn end_to_end_publish_github_if_configured() {
    let run = std::env::var("XFORGE_E2E_GITHUB").ok();
    if run.as_deref() != Some("1") {
        eprintln!("Skipping GitHub publish test (set XFORGE_E2E_GITHUB=1 to enable).");
        return;
    }
    let repo = match std::env::var("XFORGE_PUBLISH_REPO") {
        Ok(value) => value,
        Err(_) => {
            eprintln!("Skipping GitHub publish test (XFORGE_PUBLISH_REPO not set).");
            return;
        }
    };
    ...
    let private_key = match std::env::var("XFORGE_PRIVATE_KEY") {
        Ok(value) => value,
        Err(_) => {
            eprintln!("Skipping GitHub publish test (XFORGE_PRIVATE_KEY not set).");
            return;
        }
    };
