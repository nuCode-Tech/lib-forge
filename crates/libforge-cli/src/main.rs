...
#[derive(Parser)]
#[command(name = "libforge", version, about = "LibForge CLI")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Build {
        ...
        /// Target triple (overrides libforge.yaml).
        #[arg(long)]
    ...
    Keygen,
    /// Sign a file with LIBFORGE_PRIVATE_KEY and write a .sig file.
    Sign {
        /// File to sign.
        #[arg(long)]
        file: PathBuf,
    ...
    Publish {
        ...
        #[arg(long, default_value = "libforge-manifest.json")]
        manifest: PathBuf,
        /// Repository slug (owner/repo). If omitted, read from libforge.yaml.
        #[arg(long)]
...
fn run_cli() -> Result<(), String> {
    ...
    match cli.command {
        ...
        Command::Sign { file, out } => {
            let private_key_hex = std::env::var("LIBFORGE_PRIVATE_KEY")
                .map_err(|_| "Missing LIBFORGE_PRIVATE_KEY environment variable".to_string())?;
