...
use libforge_cli::commands::{build, bundle, keygen, publish};
use libforge_core::manifest::{deserialize_manifest, signing_payload};
use libforge_core::security::{parse_public_key_hex, verify};
use libforge_publish::local::LocalPublisher;
use libforge_publish::release::{publish_release, PublishRequest};
...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let stamp = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    path.push(format!("libforge-e2e-{}-{}", name, stamp));
