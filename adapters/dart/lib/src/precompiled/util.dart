...
Future<List<int>> readOrDownloadBytes(
  File file,
  Uri url, {
  Map<String, String>? headers,
}) async {
  ...
  final response = await httpGetWithRetry(url, headers: headers);
  if (response.statusCode != 200) {
    throw HttpException(
      'Failed to download ${url.toString()} (status ${response.statusCode})',
      uri: url,
    );
  }
